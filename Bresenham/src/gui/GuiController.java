...
public class GuiController {
	...
	public GuiController(GuiView guiView, GridBuilder gridBuilder) {
		...
		this.gridBuilder
				.setMousePressOnPixelHandler(new MousePressOnPixelHandler());
		this.gridBuilder
				.addMouseDragEnteredListener(new MouseDragEnteredListener());
	...
	public void buildGrid() {
		guiView.setGrid(gridBuilder.buildGrid(pixelSize, sizeInPixelX,
				sizeInPixelY));
		guiView.updateMenu();
	}
	...
	private void drawHelpline(int beginX, int beginY, int endX, int endY,
		...
		if (guiView.miShowLine.isSelected()) {
			...
			helpLine.setStartX(beginX * (pixelSize + GridBuilder.offset)
					+ pixelSize / 2);
			helpLine.setEndX(endX * (pixelSize + GridBuilder.offset)
					- pixelSize / 2);

			helpLine.setStartY(beginY * (pixelSize + GridBuilder.offset)
					+ pixelSize / 2);
			helpLine.setEndY(endY * (pixelSize + GridBuilder.offset)
					- pixelSize / 2);
	...
	private void wuLine(int x0, int y0, int xn, int dx, int dy, boolean sp) {
		...
		/*
		 * if (yn2 > xn2) { foo = yn2; bar = y0; yn2 = xn2; y0 = x0; xn2 = foo;
		 * x0 = bar; exchange = true; System.out.println("Tausch"); }
		 */

		double incr = yn2 / xn2;
		if(abs(incr)==1||abs(incr)==0){
			bres1(x0,y0,xn,dx,dy,sp);
			return;
		}
		double value = 0;
		if (!sp) {
			gridBuilder.setPixel(x0, y0, Color.BLACK);
			gridBuilder.setPixel(x0 + dx, y0 + dy, Color.BLACK);

		} else {
			gridBuilder.setPixel(y0, x0, Color.BLACK);
			gridBuilder.setPixel(y0 + dy, x0 + dx, Color.BLACK);
		...
		while (abs(x2) < abs(xn2)) {
			...
			if (!sp) {
				System.out.println("A:" + x2 + " : " + y2 + " : " + incr + " : "
						+ value);

				gridBuilder.setPixel((int) x2 + x0, (int) y2 + y0,
						GreyScaleFactory.getGreyScale(abs(value)));
				gridBuilder.setPixel((int) x2 + x0, (int) y2 + y0 +yadd,
						GreyScaleFactory.getGreyScale(abs(1 - value)));
			} else {
				System.out.println("B: " + y2 + " : " + x2 + " : " + incr + " : "
						+ value);

				gridBuilder.setPixel((int) y2 + y0, (int) x2 + x0,
						GreyScaleFactory.getGreyScale(abs(value)));
				gridBuilder.setPixel((int) y2 + y0, (int) x2 + x0 + yadd,
						GreyScaleFactory.getGreyScale(abs(1 - value)));
		...
		public void handle(MouseEvent event) {
			...
			if (event.getButton() == MouseButton.PRIMARY) {
				...
				if (beginX > -1 && beginY > -1) {
					int endX, endY;
					try {
						endX = gridBuilder.getPixel((int) event.getX(),
								(int) event.getY()).getxInMap();
						endY = gridBuilder.getPixel((int) event.getX(),
								(int) event.getY()).getyInMap();
					...
					drawHelpline(beginXorg, beginYorg, endXorg, endYorg,
							changeX, changeY);
					...
					try {
						beginX = gridBuilder.getPixel((int) event.getX(),
								(int) event.getY()).getxInMap();
						beginY = gridBuilder.getPixel((int) event.getX(),
								(int) event.getY()).getyInMap();
