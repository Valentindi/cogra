  ...
  public GridPane buildGrid(int pixelSize, int windowHeight, int windowWidth) {
    ...
    for (int i = 0; i < gridArray.length; i++) {
      for (int t = 0; t < gridArray[i].length; t++) {
        // Hier wird das gridArray erstellt und mit weißen Pixeln gefüllt
        ...
        gridArray[i][t].addEventHandler(MouseEvent.MOUSE_PRESSED, handleClickOnPixel);

        root.getColumnConstraints().add(new ColumnConstraints(pixelSize + offset));
